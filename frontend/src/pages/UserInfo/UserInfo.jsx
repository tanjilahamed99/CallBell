...
import { User, Phone, Mail, Shield, Sparkles, Ban } from "lucide-react";
    ...
    if (userID && userName) {
      const fetch = async () => {
        ...
        if (data.success) {
          ...
          // Check if this guest's phone number is in the user's blocked list
          if (gest?.phone) {
            const blocked = (data.data.blockedGest || []).some(
              (b) => b.guestNumber === gest.phone,
            );
                ...
                {isBlocked ? (
                  ...
                  <div className="text-center py-6">
                    ...
                    <p className="text-gray-500 text-sm max-w-xs mx-auto">
                      You are unable to call this user at this time.
                    </p>
                    <div className="mt-6 pt-6 border-t border-red-100">
                      <div className="flex items-center justify-center gap-2 text-sm text-gray-500">
                        <Shield className="w-4 h-4 text-red-400" />
                        <span>This user has restricted incoming calls</span>
                      </div>
                    </div>
                  </div>
                ) : (
